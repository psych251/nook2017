...
The original researchers "excluded eight participants who consistently progressed through trials without writing for a full 30 s and five who wrote about topics other than the images". Like the original study, written responses will be automatically submitted after 30 seconds. 
...
To ensure compliance with federal minimum wage, participants will be paid $1.50 for completing our survey.

### Materials

The original researchers "assembled three lists of 20 images from the Open Affective Standardized Image Set (OASIS; Kurdi, Lozano, & Banaji, 2016). One list included only neutral images [...] The other two lists were both negative, and they were matched for valence (List A: M  = 2.37, SD  = 0.42; List B: M = 2.38, SD = 0.42; item analysis t(38) = -0.12, p = .906) and arousal (List A: M = 4.24, SD = 0.56; List B: M = 4.25, SD = 0.50; t(38) = -0.06, p = .954)."
...
Our replication will follow the original authors' design, except that it will only include two lists (look negative and reappraise negative) of 10 images which will be matched for valence (List A: M = 2.32, SD = 0.41; List B: M = 2.31, SD = 0.37; t(18) = 0.08, p = .940) and arousal (List A: M = 4.33, SD = 0.55; List B: M = 4.36, SD = 0.57; t(18) = -0.11, p = .912). We will use the same standard to select and group images from OASIS; however, the specific images will differ from those used in the original study.

The replication paradigm will be set up on Qualtrics and linked for use on mTurk. 
...
We will precisely follow the original procedure as detailed above, except participants will complete 10 trials of each condition and the mapping of the lists will be counterbalanced.
...
Our replication used the same task instructions as the original study, with a few additional sentences to inform participants that their responses would submit automatically after 30 seconds and to encourage participants to pay attention. See project repository for a full side-by-side comparison.

The order that the 20 images appeared to participants was randomized.

Participants were not asked to report annual family income as originally planned, but they were asked to report their age, gender, and race at the end of the survey. These questions were not required fields.
...
df.tidy <- csv %>% 
  ...
  # Remove failed attention checks and old data.
...
Here, wrangled data was manually edited for spelling errors and exporting issues (e.g. Qualtrics coding apostrophes as other icons). Then, the edited csv was run through LIWC.
...
df.exclusion <- df.renamed

df.new_variables2 <- df.exclusion %>% 
...
df.new_variables <- df.new_variables2 %>% 
  dplyr::group_by(subject, condition) %>% 
  ...
  mutate(reappraisal = mean(look_mean_rating, na.rm=TRUE) - mean(change_mean_rating, na.rm=TRUE))
         ...
         #aggregate(rating ~ subject + condition , FUN = mean))
...
#df.new_variables$mean_rating <- paste(df.new_variables$mean_look_rating, #df.new_variables$mean_change_rating)

View(df.new_variables)
```
...
```{r}
df.linguistic_distancing2 <- df.new_variables %>% 
  #dplyr::mutate(linguistic_distancing = (pronoun_rz + verb_rz + article_z + discrepancy_rz + six_letter_z)/n()) %>% 
  dplyr::group_by(subject, condition) %>% 
  dplyr::mutate(linguistic_distancing = sum(pronoun_rz, verb_rz, article_z, discrepancy_rz, six_letter_z, na.rm = TRUE)/n())

df.linguistic_distancing <- df.linguistic_distancing2 %>% 
  dplyr::group_by(condition) %>% 
  dplyr::mutate(avg_linguistic_distancing = mean(linguistic_distancing),
...
```{r}
df.error <- summarySE(df.linguistic_distancing, measurevar="avg.linguistic_distancing")
df.error
```
...
```{r}
ggplot(df.linguistic_distancing, aes(x=condition, y=(grouped_mean_rating/560))) +
  geom_col() +
  #geom_errorbar(aes(ymin = len, ymax = len+sd), width = 0.2) +
  labs(tag = "a", x = "", y = "Self-Reported Negative Affect") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line.x = element_line(color = "black"),
        axis.line.y = element_line(color = "black")) +
  scale_x_discrete(limits = c("look", "change"), labels = c("Look Negative", "Reappraise Negative")) +
  coord_cartesian(ylim = c(0, 5)) +
  scale_y_continuous(breaks = seq(from = 0, to = 5, by = 1))
```

```{r}
ggplot(df.linguistic_distancing, aes(x=condition, y=(negative_affect/560))) +
  geom_col() +
  #geom_errorbar(aes(ymin = len, ymax = len+sd), width = 0.2) +
...
```{r}
ggplot(df.linguistic_distancing, aes(x=condition, y=(positive_affect/560))) +
  geom_col() +
  #geom_errorbar(aes(ymin = len, ymax = len+sd), width = 0.2) +
...
```{r graph}
ggplot(df.linguistic_distancing, aes(x=condition, y=avg_linguistic_distancing)) +
  geom_col(color = "gray") +
  ...
  # scale_y_continuous(breaks = seq(from = -0.2, to = 0.15, by = 0.05)) 
```
...
ggplot(df.linguistic_distancing, aes(x=linguistic_distancing, y=reappraisal)) +
  ...
  labs(tag = "b", x = "Δ Linguistic Distancing When Reappraising", y = "Reappraisal Success") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
