...
Prior to conducting the repeated-measures ANOVAs, we checked the data by identifying extreme outliers and assessing normality.
...
```
Because the Shapiro-Wilk test becomes very sensitive even to a minor deviation from normality at larger sample sizes, we created QQ plots for each condition. Our data was not normally distributed, and so we were unable to conduct the repeated-measures ANOVA (code below).
...
Our two **key analyses of interest** included:

- A paired-samples t tests (to assess for differences between the reappraise negative condition and the other two conditions).
