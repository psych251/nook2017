...
There are linguistic markers in speech which are indicative of the degree of psychological distance employed by the speaker. Nook et al. (2017) explore bidirectional relations between emotion regulation and linguistic signatures of psychological distancing.
...
Our replication only included two lists (look negative and reappraise negative) of 10 images. While the specific images we used differed from those in the original study, our image sets were matched for valence (List A: M = 2.32, SD = 0.41; List B: M = 2.31, SD = 0.37; t(18) = 0.08, p = .940) and arousal (List A: M = 4.33, SD = 0.55; List B: M = 4.36, SD = 0.57; t(18) = -0.11, p = .912), which were comparable to the original.
...
Our replication followed the original authors' procedure as detailed above.

The replication paradigm was set up on Qualtrics and linked for use on mTurk. 
...
```{r}
sum(df.linguistic_distancing$avg_linguistic_distancing)
```


```{r graph}
ggplot(df.linguistic_distancing, aes(x=condition, y=(avg_linguistic_distancing/560))) +
  geom_col(color = "gray") +
  ...
  coord_cartesian(ylim = c(-1e-17, 2e-17)) +
  scale_y_continuous(breaks = seq(from = -1e-17, to = 2e-17, by = 5e-18)) +
  annotate("text", x = 1.5, y = 2e-17, label = "***", size = 10)
```
```{r}
...
ggplot(df.linguistic_distancing, aes(x=condition, y=(grouped_mean_rating/560))) +
  ...
  annotate("text", x = 1.5, y = 5, label = "***", size = 10)
```
